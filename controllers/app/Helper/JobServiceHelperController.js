...
async function getDictionary() {
  ...
  const docs = await JopServiceModel.find({}, { name: 1, title_ar: 1, title_en: 1, keyword: 1 }).lean();
  const set = new Set();
  for (const d of docs) {
...
const search = async (req, res) => {
  try {
   ...
   if (!tokens.length) return ReturnAppData.getData({res,data:[]}); 
    ...
    let docs = await JopServiceModel.aggregate(pipeline).exec();
    ...
    if (docs.length === 0) {
      ...
      if (changed) {
        ...
        docs = await JopServiceModel.aggregate(pipeline2).exec();
      }
    }
