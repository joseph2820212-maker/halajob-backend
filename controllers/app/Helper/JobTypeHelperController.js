...
async function getDictionary() {
  ...
  const docs = await JobTypeModel.find({}, { name: 1, title_ar: 1, title_en: 1, keyword: 1 }).lean();
  const set = new Set();
  for (const d of docs) {
...
const search = async (req, res) => {
  try {
    ...
    let docs = await JobTypeModel.aggregate(pipeline).exec();
    ...
    if (docs.length === 0) {
      ...
      if (changed) {
        ...
        docs = await JobTypeModel.aggregate(pipeline2).exec();
      }
    }
...
const get=async(req,res,next)=>{
   const lan = (req.get("lan") || "en").toLowerCase();
  const data=await JobTypeModel.find();
