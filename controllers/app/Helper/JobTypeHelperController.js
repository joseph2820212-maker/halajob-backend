...
async function getDictionary() {
  ...
  const docs = await JopTypeModel.find({}, { name: 1, title_ar: 1, title_en: 1, keyword: 1 }).lean();
  const set = new Set();
  for (const d of docs) {
...
const search = async (req, res) => {
  try {
    ...
    let docs = await JopTypeModel.aggregate(pipeline).exec();
    ...
    if (docs.length === 0) {
      ...
      if (changed) {
        ...
        docs = await JopTypeModel.aggregate(pipeline2).exec();
      }
    }
...
const get=async(req,res,next)=>{
   const lan = (req.get("lan") || "en").toLowerCase();
  const data=await JopTypeModel.find();
  const results=data.map((item)=>{
    return {
      id:item._id,
      title:lan==="ar"?item.title_ar:item.title_en,
      option:item.option.map((element)=>{
        return {
          title:lan==="ar"?element.title_ar:element.title_en,
          type:element.type,
          option:element.option.map((op)=>{
            return{
            title:lan==="ar"?op.title_ar:op.title_en
            }
          })
        }
      })
