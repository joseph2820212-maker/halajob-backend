 ...
 try {
  ...
  if (!email || !password) {
   return ReturnAppData.createError({
    res,
    status: 400,
    message: msg(lan, "البريد وكلمة المرور مطلوبة.", "Email and password are required."),
   });
  }
  ...
  const isEmployeeRole =
   user.role_id?.log_to === "employee" ||
   String(user.role_id?.name || "").toLowerCase() === "employee";
  ...
  if (!isEmployeeRole) {
   return ReturnAppData.createError({
    ...
    message: msg(
     lan,
     "هذا الحساب لا يملك صلاحية الدخول كموظف.",
     "This account does not have permission to login as an employee."
    ),
   });
  }

  const employee = await EmployeeModel.findOne({ user_id: user._id })
   .populate("role_id")
   .populate("user_id", "-password -passcode -another_device_code -pending_device");
  ...
  if (!employee) {
   return ReturnAppData.createError({
    ...
    message: msg(
     lan,
     "لم يتم العثور على بيانات الموظف المرتبطة بهذا الحساب.",
     "Employee profile linked to this account was not found."
    ),
   });
  }

  if (!user.status || employee.status === false) {
   return ReturnAppData.createError({
    res,
    status: 403,
  ...
  const incomingDevice = {
   ...
   model_name:
    `${req.headers["sec-ch-ua-platform"] || "Unknown Platform"} Browser`,
