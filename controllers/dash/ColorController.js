 ...
 */
 const create = async (req, res, next) => {
  try {
    ...
    if (payload.name) {
      const exists = await ColorModel.findOne({ name: payload.name });
      if (exists) {
    ...
    const doc = await ColorModel.create(payload);
 ...
 */
 const update = async (req, res, next) => {
  try {
    ...
    const doc = await ColorModel.findByIdAndUpdate(id, updatePayload, {
      new: true,
      runValidators: true,
    });
 ...
 const get = async (req, res, next) => {
  ...
  try {
    ...
    const [items, total] = await Promise.all([
      ColorModel.find(filter).sort(sort).skip(skip).limit(Number(limit)),
