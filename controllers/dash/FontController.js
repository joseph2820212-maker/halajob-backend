 ...
 */
 const create = async (req, res, next) => {
  ...
  const file = req.file;
    ...
    if (payload.name) {
      const exists = await FontModel.findOne({ name: payload.name });
      if (exists) {
     ...
     if (file) {
      payload.file = file.path.replace("uploads\\", ""); // ضمان استخدام الخط المائل للأمام
    }
    const doc = await FontModel.create(payload);
 ...
 */
 const update = async (req, res, next) => {
  ...
  const file = req.file;
    ...
    if (file) {
      payload.file = file.path.replace("uploads\\", "");
    }
 ...
 const get = async (req, res, next) => {
  ...
  try {
    ...
    const [items, total] = await Promise.all([
      FontModel.find(filter).sort(sort).skip(skip).limit(Number(limit)),
      FontModel.countDocuments(filter),
    ]);
 ...
 */
 const getOne = async (req, res, next) => {
  try {
    const { id } = req.params;
    const doc = await FontModel.findById(id);
    if (!doc) {
      return ReturnDashData.getError({
        res,
        message: "Role not found",
 ...
 */
 const remove = async (req, res, next) => {
  try {
    const { id } = req.params;
    const doc = await FontModel.findByIdAndDelete(id);
    if (!doc) {
