...
const create = async (req, res, next) => {
  try {
    ...
    if (payload.name) {
      const exists = await WorkTimeTypeModel.findOne({ name: payload.name });
      if (exists) {
        return ReturnDashData.createError({
          res,
          message: "Jop Name with this name already exists",
        });
      }
    }

    const doc = await WorkTimeTypeModel.create(payload);
...
const update = async (req, res, next) => {
  try {
    ...
    const doc = await WorkTimeTypeModel.findByIdAndUpdate(
      id,
 ...
 const get = async (req, res, next) => {
  try {
    ...
    const [items, total] = await Promise.all([
      WorkTimeTypeModel.find(filter).sort(sort).skip(skip).limit(Number(limit)),
      WorkTimeTypeModel.countDocuments(filter),
    ]);
 ...
 */
 const getOne = async (req, res, next) => {
  try {
    const { id } = req.params;
    const doc = await WorkTimeTypeModel.findById(id);
    if (!doc) {
      return ReturnDashData.getError({
        res,
        message: "Jop Name not found",
 ...
 */
 const remove = async (req, res, next) => {
  try {
    const { id } = req.params;
    const doc = await WorkTimeTypeModel.findByIdAndDelete(id);
    if (!doc) {
