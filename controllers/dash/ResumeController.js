 ...
 */
 const create = async (req, res, next) => {
  ...
  const file = req.file;
    ...
    if (payload.name) {
      const exists = await ResumeModel.findOne({ name: payload.name });
      if (exists) {
    ...
    const doc = await ResumeModel.create(payload);
 ...
 const get = async (req, res, next) => {
  ...
  try {
    ...
    const [items, total] = await Promise.all([
      ResumeModel.find(filter).sort(sort).skip(skip).limit(Number(limit)),
 ...
 */
 const remove = async (req, res, next) => {
  try {
    const { id } = req.params;
    const doc = await ResumeModel.findByIdAndDelete(id);
    if (!doc) {
