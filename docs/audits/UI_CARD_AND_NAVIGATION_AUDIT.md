...
| Job seeker + campus student shell | `mobile/lib/src/features/dashboard/dashboard_screen.dart` | `DashboardScreen`, `_DashboardWorkflowPage`, `_HalaBottomNav` | Native dashboard with bottom nav and workflow pages. | Good base, but many edit components still use `Sheet` naming and one actual modal filter remains. |
...
| University/admin shell | `mobile/lib/src/features/university/university_dashboard_screen.dart` | `UniversityDashboardScreen`, `_UniversityWorkflowPage`, `_UniversityHeaderMenuButton` | Native dashboard/workflow pages. | Good base; needs page-by-page action audit. |
...
| P1 | University detail pages do not consistently use `HalaNativeHeader`. | `_StudentPassportScreen`, `_PartnerDetailsScreen`, `_OpportunityDetailsScreen`, `_OutcomesDetailsScreen`, and `_OpportunityRequestScreen` still use plain `AppBar`. | Convert to the shared navy workflow/header pattern. |
| P1 | Brand docs conflict with implemented theme. | `mobile/docs/brand/README.md` lists older colors (`#13315C`, `#F08A3C`, `#FAF9F5`), while `app_theme.dart` uses `#1F3654`, `#E38B3C`, `#FCF7EF`; card radius handout says 20px while app defaults to 8. | Decide canonical tokens, then update either docs or implementation. |
...
2. APK version or build number changes.
