...
| Auth and account-context safety | 8.82 | Seeded auth/context integration now covers missing/malformed/expired app and admin tokens, inactive app-user denial, role/context denial, cross-role borrowing, suspended and removed explicit contexts, tampered cross-user and removed stored default contexts, removed active-context selection denial, invalid context, direct refresh-token revocation, and app/company/admin logout invalidating old sessions. |
...
| Automated backend test depth | 8.55 | Static checks plus multiple seeded runtime integration harnesses exist, including high-risk auth/context/logout negative cases, company profile-file lifecycle coverage, company/university member lifecycle/context sync, student verification document privacy, company export rejection behavior, seeker job mutations, ATS/interview/invitation workflows, and campus/university workflows; still short of route-by-route validator/schema and full journey coverage. |
| Launch confidence from backend/API only | 8.71 | Stronger than the original audit, but not yet a 9+ launch certificate until remaining edge-case and live-smoke gaps are closed. |
...
| Translation consumption still needs UI/product QA | Job/CV/Career Passport translation save, owner-scoped read, draft review, approval, audit logging, analytics, and approved backend consumption now have seeded runtime coverage. Broader UI/product QA still needs to confirm approved translations display exactly where intended across every consuming screen. |
| Campus event/content management is thin | Managed campus events now have scoped university-admin list/create/update/archive APIs, audit logs, mobile-feed consumption, and seeded lifecycle coverage. Broader campus content/resource administration beyond events still needs product/UI QA and follow-up. |
| University team/member management now has an app API baseline | `/university/v1/members` and `/campus/v1/admin/members` now support scoped list/create/update/remove with role-based university permissions, account-context sync, cross-university denial, last-owner protection, and audit logs. The web campus university portal now consumes list/create/update/activate/suspend/remove with route-contract tests; remaining work is mobile UI consumption and richer staffing policy. |
...
| Newer operational records are not all admin-manageable | Examples still include richer analytics event deep-dive surfaces and broader UI workflows for AI requests/limits/content translations. Career passports now have dashboard-admin list/detail/filter read routes guarded by `career_passports.view` plus web admin reachability; invoices have dashboard-admin list/detail/filter read routes guarded by `subscriptions.manage` plus web admin reachability; support tickets, university memberships, and admin notification sending also have explicit workflow routes. |
...
| Public uploads route | `/uploads/files/*` is now blocked from public static serving, non-image root uploads are forced to attachment, student verification documents are moved into private storage with scoped download routes, trust evidence accepts only public HTTPS links rather than public upload paths, and dashboard protected file downloads require `files.read`, allow image/PDF extensions only, and serve PDFs as attachment/no-store/nosniff. Company request files, company dashboard profile files, student verification documents, CV uploads, seeker/company profile image uploads, and company logo/cover media uploads now have seeded MIME-mismatch and oversize rejection coverage; company dashboard profile files also have valid upload/delete lifecycle coverage. Remaining work is route-by-route MIME/size/private download coverage for the remaining file families. |
...
| Logout/session invalidation | Improved: user, company, and admin logout behavior now has backend handling and security-contract checks; seeded auth/context coverage proves real logout deletes refresh rows and old access tokens fail; web logout calls backend before clearing local state. |
...
| Negative authorization tests | Baseline exists for wrong role, context borrowing, suspended/removed explicit contexts, tampered cross-user and removed stored default contexts, removed active-context selection denial, expired access tokens, inactive app users, app/company/admin logout session revocation, support-ticket view/manage boundaries, company member missing-permission cases, dashboard admin missing-permission cases, company/university/student IDOR, and revoked refresh sessions; remaining gaps include uncommon inactive-context permutations beyond stored-default tampering, production support-role staffing, and workflow-specific side effects. |
| Mutation side-effect tests | Coverage exists for several object-scope, support, translation approval, app job list/detail consumption of approved job translations, CV library consumption of approved CV translations, Career Passport consumption of approved profile translations, company member create/promote/remove, job seeker save/apply/rate/review/report, ATS/interview/invitation, managed campus event/opportunity/verification, and admin-resource mutations; still needed for richer campus admin content beyond event CRUD and remaining approved-translation presentation beyond the covered job/CV/Career Passport consumers. |
| Upload/download security tests | Company request files, company dashboard profile-file upload/delete/download lifecycle, dashboard protected files, saved employee CV downloads, generated-CV safety headers/token-expiry policy, profile/company media upload rejection, high-risk upload MIME/size rejection, and trust evidence public-URL validation are covered; still needed for export files, remaining upload endpoints, all remaining private file routes, and direct trust evidence file uploads if that product path is introduced. |
...
| P0 | Add remaining inactive-context edge-case tests beyond stored-default tampering and keep new admin feature routes tied to explicit permission keys. |
...
| P1 | Add mutation tests for richer campus admin content management beyond managed event CRUD, translation publishing/consumption, and any application status journeys beyond the now-covered ATS interview/invitation loop. |
| P1 | Add upload/download security tests for remaining upload endpoints, remaining private file routes, and direct trust evidence file uploads if that product path is introduced. Student verification documents now have scoped private-storage coverage and upload validation coverage, generated CV public links now enforce token/expiry for DB-backed records, covered upload families including company profile files and profile/company media reject MIME/size failures cleanly, company profile files have upload/delete lifecycle coverage, company export endpoints reject invalid explicit application IDs and unsupported formats without success audit rows, and trust evidence links now reject private/internal public-HTTPS bypasses. |
| P1 | Add admin UI coverage or richer deep-dive surfaces for newer records: analytics event deep-dives and broader AI request/limit/content-translation operations. Student verification, university membership, career passport read, and invoice read workflows now have app/API baselines; Career Passport and invoice readers now also have web admin reachability and path-contract tests. |
| P1 | Complete AI persistence workflows where product expects generated content to become real saved records. |
| P1 | Continue translation publish/read workflow QA beyond the now-covered job/CV/Career Passport backend consumers. |
