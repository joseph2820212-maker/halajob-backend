...
| Newer operational records are not all admin-manageable | Examples still include analytics event deep-dive surfaces and UI placement for AI requests/limits/content translations. Career passports now have dashboard-admin list/detail/filter read routes guarded by `career_passports.view`; invoices have dashboard-admin list/detail/filter read routes guarded by `subscriptions.manage`; support tickets, university memberships, and admin notification sending also have explicit workflow routes. |
...
| P1 | Add admin UI coverage or richer deep-dive surfaces for newer records: career passports, AI requests/limits, analytics events, content translations, and UI surfaces for the newly exposed admin invoice readers. Student verification, university membership, career passport read, and invoice read workflows now have app/API baselines. |
