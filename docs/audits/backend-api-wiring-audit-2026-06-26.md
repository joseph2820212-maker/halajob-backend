...
| Negative authorization tests | Baseline exists for wrong role, context borrowing, suspended/removed explicit contexts, removed active-context selection denial, expired access tokens, inactive app users, company member missing-permission cases, dashboard admin missing-permission cases, company/university/student IDOR, and revoked refresh sessions; remaining gaps include uncommon inactive-context permutations, support-role policy, and workflow-specific side effects. |
