...
| P1 | Add upload/download security tests for remaining export edge cases, remaining upload endpoints, remaining private file routes, and direct trust evidence file uploads if that product path is introduced. Student verification documents now have scoped private-storage coverage and upload validation coverage, generated CV public links now enforce token/expiry for DB-backed records, covered upload families reject MIME/size failures cleanly, and trust evidence links now reject private/internal public-HTTPS bypasses. |
