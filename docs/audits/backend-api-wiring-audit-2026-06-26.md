...
| Upload/download security tests | Company files, dashboard protected files, saved employee CV downloads, generated-CV safety headers, and trust evidence public-URL validation are covered; still needed for generated CV public-link expiry/ownership policy, export files, MIME rejection, size rejection, all remaining private file routes, and direct trust evidence file uploads if that product path is introduced. |
...
| P1 | Add upload/download security tests for remaining export edge cases, generated CV expiry/ownership policy, MIME rejection, size rejection, remaining private file routes, and direct trust evidence file uploads if that product path is introduced. Student verification documents now have scoped private-storage coverage, and trust evidence links now reject private/internal public-HTTPS bypasses. |
