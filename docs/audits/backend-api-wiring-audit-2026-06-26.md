...
| Upload/download security tests | Company files, dashboard protected files, saved employee CV downloads, generated-CV safety headers/token-expiry policy, profile image upload rejection, high-risk upload MIME/size rejection, and trust evidence public-URL validation are covered; still needed for export files, remaining upload endpoints, all remaining private file routes, and direct trust evidence file uploads if that product path is introduced. |
