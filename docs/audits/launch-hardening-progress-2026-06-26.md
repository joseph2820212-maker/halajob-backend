...
| Logout security contract | HTTP contract proves user, company, and admin logout reject missing refresh tokens, and seeded auth/context coverage proves real app/company/admin logout deletes refresh rows and invalidates old access tokens. | `npm run test:security-http`, `npm run test:integration:auth-context` |
...
| Seeded auth/context integration coverage | Runtime MongoDB integration coverage now includes missing/malformed/expired app and admin tokens, inactive app-user denial, seeker, student/campus, approved company, pending company, university admin, dashboard admin, cross-role denial, cross-company context denial, suspended context denial, removed context denial, removed active-context selection denial, invalid context id, direct refresh-token revocation, and real app/company/admin logout session invalidation. | `scripts/verifyAuthContextIntegration.js` |
...
| Company file/export audit logging | Company dashboard profile file downloads, app company request file downloads, single application CV downloads, bulk CV ZIP exports, and application exports now write audit logs, with seeded runtime tests also proving dashboard profile-file MIME/size rejection, valid upload/delete lifecycle, path traversal, cross-company app file access, invalid explicit application IDs, and unsupported export formats are rejected safely. | `npm run test:file-export-audit`, `controllers/app/Company/CreateCompanyController.js`, `controllers/companyDash/companyWithJobs/companyJobHiringController.js`, `controllers/companyDash/information/companyInformationController.js` |
...
| Generated CV public-link hardening | Newly generated DB-backed CV public links now include an unguessable token and expiry, missing/wrong/expired/no-token links are rejected, and authenticated owners keep `/employee/v1/cv/download/:cvId` downloads with private/no-store/nosniff headers. | `npm run test:integration:employee-cv-downloads`, `app.js`, `controllers/employeeDash/cv/generateCvController.js`, `models/EmployeeCvModel.js` |
| Upload validation error handling | Multer upload failures now return clean `unsupported_file_type`/`file_too_large` 4xx responses, and seeded tests prove rejected company request files, company dashboard profile files, student verification documents, CV uploads, seeker/company profile image uploads, and company logo/cover media uploads do not mutate records. | `npm run test:file-export-audit`, `npm run test:integration:student-verification-documents`, `npm run test:integration:employee-cv-downloads`, `npm run test:integration:profile-uploads`, `middlewares/error.js` |
| Trust evidence URL hardening | Company trust document responses now accept only public HTTPS evidence URLs and reject localhost, private IP ranges, link-local metadata addresses, single-label intranet hosts, and embedded credentials without mutating the trust response. | `npm run test:integration:trust-documents`, `controllers/trust/TrustController.js`, `scripts/verifyTrustDocumentWorkflowIntegration.js` |
| Student verification document privacy | Student verification document uploads now move into private `uploads/files/student-verifications` storage, direct public access is denied, student/university-admin download routes are scoped, attachment/no-store/nosniff headers are sent, and upload/download audit rows are verified. | `npm run test:integration:student-verification-documents`, `scripts/verifyStudentVerificationDocumentSecurity.js` |
...
| Company member lifecycle/context coverage | Seeded runtime coverage now proves member create/promote/remove, owner self-add denial, account-context sync after role changes, removed-context denial, member-managed invites, cross-company denial, and audit actor types. | `npm run test:integration:company-members`, `scripts/verifyCompanyMemberWorkflowIntegration.js` |
| University member lifecycle/context coverage | University team management APIs now exist under `/university/v1/members` and `/campus/v1/admin/members`; seeded runtime coverage proves create/promote/remove, role-based permissions, account-context sync, removed-context denial, campus-admin aliases, cross-university denial, last-owner protection, and audit logs. | `npm run test:integration:university-members`, `scripts/verifyUniversityMemberWorkflowIntegration.js` |
...
| Admin support workflow coverage | Dashboard support ticket list/detail/status/reply endpoints now exist with `support.view` and `support.manage` boundaries, operations aliases, assignment/closure fields, admin replies, and audit logging. `support.manage` supervisors can read queue/detail and mutate tickets; `support.view` remains read-only. | `npm run test:integration:admin-support`, `scripts/verifyAdminSupportIntegration.js` |
...
The project still needs the larger runtime integration suite from the backend audit: remaining non-admin mutation side effects, remaining admin audit-log assertions, protected download coverage for every private file route, any future external payment-provider callbacks, route-by-route schemas/validators, and full web/mobile end-to-end API flow coverage. AI, notifications including preferences/admin-send, analytics, subscription/billing, company member permissions, company member lifecycle/context sync, university member lifecycle/context sync, admin permission boundaries, admin support workflows, translation save/read/approval, admin resource redaction/mutation audits, generated CV public-link hardening, high-risk upload validation including profile/company media/company profile-file rejection, trust evidence URL hardening, student verification document privacy, employee saved-CV downloads, job seeker mutation workflows, ATS/interview/invitation workflows, campus/university workflows, and audit-log secret redaction now have seeded runtime coverage for their core request/report paths.
