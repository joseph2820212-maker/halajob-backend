...
| Upload validation error handling | Multer upload failures now return clean `unsupported_file_type`/`file_too_large` 4xx responses, and seeded tests prove rejected company request files, student verification documents, and CV uploads do not mutate records. | `npm run test:file-export-audit`, `npm run test:integration:student-verification-documents`, `npm run test:integration:employee-cv-downloads`, `middlewares/error.js` |
