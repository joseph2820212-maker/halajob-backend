...
| `npm run test:file-export-audit` | Passed, including company request upload MIME/size rejection, dashboard company profile file MIME/size rejection, no mutation on rejected uploads, valid company profile file upload/delete lifecycle, dashboard company file download, app company request file download, traversal rejection, other-company denial, bulk export invalid-ID/format rejection, no successful export audit row for rejected export requests, and export audit logs |
