...
| `npm run test:integration:employee-cv-downloads` | Passed, including generated-CV public missing/wrong/expired/no-token rejection, valid public token download headers, saved-CV auth, owner-only download, cross-user denial, invalid-ID rejection, unsafe-path rejection, and missing-file handling |
