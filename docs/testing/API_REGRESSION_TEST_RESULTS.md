...
| `npm run test:file-export-audit` | Passed, including company request upload MIME/size rejection, no mutation on rejected uploads, dashboard company file download, app company request file download, traversal rejection, other-company denial, bulk export invalid-ID/format rejection, no successful export audit row for rejected export requests, and export audit logs |
| `npm run test:integration:profile-uploads` | Passed, including seeker/company profile image and company logo/cover media MIME/size rejection with no image-field mutation on rejected uploads |
...
| `npm run test:integration:admin-support` | Passed, including support.view read-only access, support.manage queue/detail read access, support.manage mutation access, support queue/detail operations aliases, status validation, assignment, admin replies, close behavior, and audit logs |
...
| `npm run test:integration:employee-cv-downloads` | Passed, including CV upload MIME/size rejection, no mutation on rejected uploads, generated-CV public missing/wrong/expired/no-token rejection, valid public token download headers, saved-CV auth, owner-only download, cross-user denial, invalid-ID rejection, unsafe-path rejection, and missing-file handling |
...
| `npm run test:integration:auth-context` | Passed, including missing/malformed/expired app tokens, expired admin tokens, inactive app-user denial, role/context denial, borrowed-context denial, suspended-context denial, removed-context denial, removed active-context selection denial, invalid-context rejection, pending-company denial, and revoked refresh-session denial |
