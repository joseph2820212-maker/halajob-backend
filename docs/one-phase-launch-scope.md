...
| Translation | Backend translation approval storage is now implemented with `/jobs/v1/:jobId/translations/:lang` and `/user/v1/cv/translations/:lang`, preserving original text, translated text, language direction, AI request id, status, approver, and approval time. AI generation endpoints remain safety-gated until a provider is enabled. | Add admin review UI, mobile edit/approve screens, provider credentials, and production QA for Arabic/English outputs. |
...
| Push notifications | Versioned `/notifications/v1` list/read/device-token routes now wrap the existing notification and FCM-token foundation. Launch notification event catalog now covers application status, interviews, employer applicant receipt, company profile views, matching jobs, campus verification, campus events, CV export, and async AI results. Campus verification approve/reject/request-info, campus event registration, and scheduled campus event reminders now create student notifications. The Flutter shell can initialize Firebase Messaging from Dart defines, register the native token after sign-in/session restore, re-register on Firebase token refresh, route cold/background notification taps into seeker/campus/company dashboard destinations, and revoke the token on sign-out. Local builds without Firebase config still run normally. | Finish production Firebase credential QA, physical-phone token issuance, push delivery, real-device tap routing, and duplicate-notification checks. |
| Analytics | Backend `/analytics/v1/events` foundation now records the approved activation, AI, jobs, company, campus, and global events against the signed-in user/context. Signup/login completion, account-context switching, profile completion, country/currency preference changes, job view/save/apply, remote/hybrid filter usage, campus event registration, campus verification start/approval, company profile updates, job create/publish, candidate shortlist, interview scheduling, CV/application exports, completed/cached AI copilot/score/CV/job-match/cover-letter/interview/shortlist/job-draft/hiring-message results, and Career Passport score refreshes now emit automatic analytics events with verifier coverage. The Flutter shell now sends best-effort mobile `login_completed` and `account_context_switched` activation events, with `/analytics/v1/track` fallback and local campus tester-token skip behavior. Guarded `/analytics/v1/admin/summary` and `/analytics/v1/admin/cohorts` report endpoints are mounted for super admin and scoped university admin contexts. | Build frontend/admin reporting screens on these endpoints and keep translation analytics tied to human-approved publish/save actions. |
| Payments/subscriptions | Manual/admin subscriptions are implemented: companies can read subscription/billing/invoices, submit plan-change requests, and admins can seed/assign plans with feature/limit snapshots. Integration coverage verifies permissions, invoice ownership, support-ticket/audit creation, admin assignment, and missing-plan failure. | Owner must explicitly accept manual/admin subscriptions for launch or select an online payment provider. Online checkout, webhook signature validation, payment sync, failed-payment handling, refunds, and provider sandbox proof are not implemented until the provider is selected. |
...
- Dashboard admin endpoints now expose `/dash/v1/ai/features`,
  `/dash/v1/ai/limits`, `/dash/v1/ai/requests`, and `/dash/v1/ai/summary` so
  dashboard admins can inspect requests, review errors, check token/cost
  estimates, and manage feature enablement/limits without direct database edits.
- Provider generation is disabled by default, but the backend now has a
  provider adapter foundation for `mock`, `openai`, and `openai_compatible`.
  Mock output is explicit and blocked in production unless deliberately allowed;
  live providers require backend credentials and return normalized JSON output
  under the declared feature output keys.
...
- Flutter company More now includes AI tools for `company_job_draft`,
  `company_candidate_shortlist`, and `company_hiring_message`; company job
  detail also exposes shortlist explanation for the selected job.
- All Flutter AI surfaces call only the backend AI routes, visibly label
  results as AI suggestions, and surface blocked/provider-disabled responses
  as safe product states.
...
| AI | `ai_copilot_used`, `ai_score_generated`, `ai_cv_rewritten`, `ai_job_match_viewed`, `ai_cover_letter_generated`, `ai_interview_practiced`, `ai_shortlist_generated`, `ai_job_draft_generated`, `ai_hiring_message_generated` |
...
- Manual/admin subscription launch acceptance, or online payment provider setup if card checkout is required before launch.
