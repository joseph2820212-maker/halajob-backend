...
- `powershell -ExecutionPolicy Bypass -File mobile\scripts\build-android.ps1 -BuildTarget release-apk-local -BaseUrl https://jobzain.com -LocalCampusAuth`: built `1.0.2+20` from commit `aa65e9e`
- `adb install -r C:\Users\Admin\Documents\Codex\2026-06-28\ca\outputs\halajob-mobile-campus-tester-latest.apk`: installed the current tester APK on `emulator-5554`; sign-in, campus tester entry, campus dashboard, and seeker/company text input screenshots captured on 2026-06-28
...
- Current PC emulator proof shows the Campus role selector visible, selected, and able to open the native campus dashboard through `Use campus tester account` without typing credentials.
