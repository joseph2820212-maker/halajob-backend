...
} = await importLocal("services/content/legalContentEnforcement.service.js");
...
const missing = REQUIRED_PRODUCTION_LEGAL_PAGE_KEYS.filter((k) => !pageKeys.has(k));
if (missing.length) fail(`missing legal pages: ${missing.join(", ")}`);
else ok(`all ${REQUIRED_PRODUCTION_LEGAL_PAGE_KEYS.length} legal pages present`);
const legalMode = legalContentEnforcementMode();
if (legalMode === "production") {
  const approvalFailures = productionLegalApprovalFailures(pages);
  if (approvalFailures.length) {
    fail(
      `production legal approvals missing: ${approvalFailures
        .map((item) => `${item.key} (${item.status})`)
        .join(", ")}`,
    );
  } else {
    ok("production legal approvals complete");
  }
} else {
  ok("legal content enforcement mode: staging");
}
